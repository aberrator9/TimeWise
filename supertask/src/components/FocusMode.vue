    ...
    <div v-for="task in tasksNow">
        <div
            ...
            <div class="flex justify-between">
                <p class="my-2 ml-4"> {{ (task.subtasks && task.subtasks.length) ? task.subtasks[displayedIdx].name : '' }}
                </p>
                <button class=" end-0 block" @click="rerollSubtask(task.subtasks.length)"
            ...
            <p class="ml-2 mt-6 mb-3">For {{ getRemaining(task) }} more {{ units }}</p>
            <div class="w-[90%] h-0.5 my-1 ml-[5%]">
                <div class="shadow w-full bg-zinc-900 h-0.5">
                    <div class="bg-red-400 leading-none text-center text-white h-0.5" style="width: 75%"></div>
                </div>
            </div>
        </div>
    </div>
